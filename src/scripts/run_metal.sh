...
SYSTEM={YOUR_SYSTEM} # supported system: Metal, Metal-v, Metal-c, Metal-s
...
screen -dmS $EXP_NAME bash -c "cd $WORKING_DIR; python3 -u ../src/MultiAgentSystem.py --exp_name $EXP_NAME --max_iter $MAX_ITER --model $MODEL --data_range $DATA_RANGE --dataset_dir $DATASET_DIR --working_dir $WORKING_DIR --n_process $N_PROCESS --agent_kwargs '$AGENT_KWARGS' 2> $LOG_FILE_PATH"