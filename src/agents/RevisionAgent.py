...
class RevisionAgent(Agent):
  def __init__(self, model, **kwargs):
    super().__init__("RevisionAgent", model, **kwargs)
  ...
  def revision_prompt_assembler(self, code):
    from agents.prompts.revision_prompt import code_revision_prompt
    prompt = code_revision_prompt.format(code=code)
    return prompt
  
  def act(self, groundtruth_img_path, generated_code, output_path, output_prefix, output_suffix):
    prompt = self.revision_prompt_assembler(generated_code)
