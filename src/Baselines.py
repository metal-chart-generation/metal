...
from agents.SelfRevisionAgent import SelfRevisionAgent
from agents.prompts.hint_enhanced_prompt import hint_enhanced_prompt_template as hint_enhanced_prompt
...
def process_data(data_item, config, model, max_iterations):
    ...
    try:
        ...
        for j in range(1, max_iterations + 1):
            ...
            for lowest_metric in revision_rank:
                try:
...
def hint_enhanced_process_data(data_item, config, model, max_iterations):
    log = []
    
    output_suffix = os.path.splitext(os.path.basename(data_item["GroundTruthFigure"]))[0]
    output_prefix = "hint_enhanced_direct_generation"
    output_path = f"{config['working_dir']}{output_suffix}/"
    os.makedirs(output_path, exist_ok=True)
    ...
    try:
        ...
        generation_prompt = generation_prompt.split("\nNow")[0] + "\n" + hint_enhanced_prompt + "\nNow" + generation_prompt.split("\nNow")[1]
        ...
        """
        Hint Enhanced Generation
        """
        logger.info("  |-- Hint Enhanced Generation")
        ...
        logger.info("  |-- Hint Enhanced Evaluation")
        generated_code_evaluate_path = f"{os.path.splitext(generated_code_path)[0]}_evaluate.py"
        evaluation_results, evaluate_revision_rank = evaluation_agent.act(
            ground_truth_code_path, generated_code_evaluate_path)
        log_entry = {
        ...
        if os.path.exists(error_file_path):
            ...
            logger.info("  |-- Hint Enhanced Generation (Try Again)")
            ...
            logger.info("  |-- Hint Enhanced Evaluation (Try Again)")
            generated_code_evaluate_path = f"{os.path.splitext(generated_code_path)[0]}_evaluate.py"
            evaluation_results, evaluate_revision_rank = evaluation_agent.act(
                ground_truth_code_path, generated_code_evaluate_path)
            log_entry = {
    ...
    return {"data": output_suffix, "log": log, "flag": "hint_enhanced", "improvement": 0}


def self_revision_process_data(data_item, config, model, max_iterations):
    log = []
    ...
    try:
        ...
        agents = {
            "GenerationAgent": GenerationAgent(model, **config.get('agent_kwargs', {})),
            "SelfRevisionAgent": SelfRevisionAgent(model, **config.get('agent_kwargs', {})),
            "VerificationAgent": VerificationAgent(model, **config.get('agent_kwargs', {})),
            "EvaluationAgent": EvaluationAgent(model, **config.get('agent_kwargs', {})),
        }
        generation_agent = agents["GenerationAgent"]
        self_revision_agent = agents["SelfRevisionAgent"]
        verification_agent = agents["VerificationAgent"]
        evaluation_agent = agents["EvaluationAgent"]
        ...
        for j in range(1, max_iterations + 1):
            ...
            try:
                ...
                """
                logger.info(f"     |----- [iteration {j}] Revision")
                revision_prompt, revision_raw_response, revised_code, revised_code_path, revised_img_path, revised_pdf_path, revised_combined_image_path = self_revision_agent.act(
                    ground_truth_img_path, generated_code, output_path, output_prefix, output_suffix)
                logger.info(format_log_critique(revision_raw_response))
                ...
                evaluation_overall_changed = 0
                for metric, value in new_evaluation_results.items():
                    old_value = evaluation_results.get(metric, 0)
                    changed = value - old_value
                    evaluation_overall_changed += changed
                    logger.info(f"            >>> {metric}: {value} ({'+' if changed >=0 else ''}{changed})")
                
                evaluation_improved = True if evaluation_overall_changed > 0 else False
...
def best_of_n_process_data(data_item, config, model, max_iterations):
    log = []
    ...
    with open(f"{output_path}{output_suffix}_log.json", "w") as f:
        json.dump(log, f, indent=2)
    return {"data": output_suffix, "log": log, "flag": "best_of_n" , "improvement": "best_of_n"}
...
class Baselines:
    ...
    def run(self):
        ...
        with ProcessPoolExecutor(max_workers=num_workers, mp_context=ctx) as executor:
            if self.system == "HintEnhanced":
                func = partial(hint_enhanced_process_data, config=self.config, model=self.model, max_iterations=self.max_iterations)
            elif self.system == "Best-of-N":
                func = partial(best_of_n_process_data, config=self.config, model=self.model, max_iterations=self.max_iterations)
            elif self.system == "SelfRevision":
                func = partial(self_revision_process_data, config=self.config, model=self.model, max_iterations=self.max_iterations)
            else:
...
if __name__ == "__main__":
    ...
    # System:
    # 1. HintEnhanced: GenerationAgent + HintEnhanced Prompt, EvaluationAgent
    # 2. SelfRevision: GenerationAgent, SelfRevisionAgent, EvaluationAgent
    # 3. Best-of-N: GenerationAgent, EvaluationAgent
    ...
    baselines = Baselines(model, data, n_process, config)
    baselines.run()
